...
using GameECS.Modules.Combat.Server.Systems;
...
namespace GameECS.Modules.Combat.Server;
...
public sealed class ServerCombatModule : IDisposable
{
    ...
    /// </summary>
    public event Action<DamageNetworkMessage>? OnDamageDealt;

    /// <summary>
    /// Evento disparado quando uma entidade morre.
    /// </summary>
    public event Action<DeathNetworkMessage>? OnEntityDeath;
    ...
    private void OnDamageDealtInternal(DamageNetworkMessage msg) => OnDamageDealt?.Invoke(msg);
    private void OnEntityDeathInternal(DeathNetworkMessage msg) => OnEntityDeath?.Invoke(msg);
    private void OnDeathInternal(int entityId, int killerId, long tick) 
        => OnEntityDeath?.Invoke(new DeathNetworkMessage { EntityId = entityId, KillerId = killerId, ServerTick = tick });
