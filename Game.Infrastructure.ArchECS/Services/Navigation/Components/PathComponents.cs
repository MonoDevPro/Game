...
public struct NavPathBuffer
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public Position GetCurrentWaypointAsPosition(int gridWidth, int gridHeight)
    {
        int idx = GetCurrentWaypoint();
        if (idx < 0) return new Position { X = -1, Y = -1 };
        return new Position { X = idx % gridWidth, Y = idx / gridWidth };
    }
