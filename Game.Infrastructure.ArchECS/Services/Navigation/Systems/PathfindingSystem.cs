    ...
    WorldMap mapData)
{
        ...
        PathfindingContext ctx)
    {
        ...
        // Validação rápida
        if (!IsValidCoord(request.StartX, request.StartY, request.StartFloor) ||
            !IsValidCoord(request.GoalX, request.GoalY, request.GoalFloor) ||
            !IsWalkable(request.StartX, request.StartY, request.StartFloor) || 
            !IsWalkable(request.GoalX, request.GoalY, request.GoalFloor))
        {
        ...
        int generation)
    {
        for (int i = 0; i < 8; i++)
        {
            ...
            int nz = 0; // Suporte a múltiplos andares pode ser adicionado aqui
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private int CoordToIndex(int x, int y) => y * mapData.Width + x;
