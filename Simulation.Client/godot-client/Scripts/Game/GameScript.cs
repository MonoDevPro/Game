...
using GodotClient.Game.Simulation;
...
public partial class GameScript : Node2D
{
    ...
    // Opcional: runtime de mapa (dados + spatial + cache), caso você integre navegação/predição clientside
    ...
    private void LoadGameData()
    {
        ...
        var mapService = mapSnap.Value.ToMapService(id: mapSnap.Value.Name.GetHashCode(), usePadded: false, borderBlocked: true);
        var spatial = SpatialFactory.FromMapBounds(mapService);
        var cache = mapService.BuildCollisionCache();
        _mapRuntime = new MapRuntime(mapService, spatial, cache);
    ...
    private void HandlePlayerSpawn(INetPeerAdapter peer, ref PlayerSnapshot snapshot)
    {
        ...
        var stats = snapshot.Stats; // TODO: ajuste conforme a definição de PlayerSnapshot
