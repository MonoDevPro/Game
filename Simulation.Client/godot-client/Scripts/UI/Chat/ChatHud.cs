...
public partial class ChatHud : Control
{
    ...
    public void AppendMessage(ChatMessage packet)
    {
    ...
    private static Label CreateMessageLabel(ChatMessage packet)
    {
        var timestamp = Time.GetDatetimeStringFromSystem(utc: false, useSpace: true).Substring(11, 8);

        var source = $"[{timestamp}] {packet.Channel}] {packet.Sender}: {packet.Message}";
