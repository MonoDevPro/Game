    ...
    int mapHeight)
{
        ...
        PathfindingContext ctx)
    {
        ...
        int generation = ctx.Generation; // ← Captura geração atual
        ...
        int generation)
    {
        for (int i = 0; i < 8; i++)
        {
            ...
            if (i % 2 == 1) // Direções diagonais são índices ímpares
            {
            ...
            // Verifica se é um nó novo nesta geração ou se encontrou caminho melhor
            ...
            if (isNewNode || tentativeG < neighbor.GCost)
            {
                ...
                // TODO: Se precisar atualizar prioridade de nó existente,
                // implementar decrease-key no heap
            }
        }
    }
    ...
    private PathResult ReconstructPath(PathfindingContext ctx, int goalIndex, Span<int> pathBuffer, int generation)
    {
        ...
        while (current != -1 && length < pathBuffer.Length)
        {
            ...
            if (node.Generation != generation) break; // Segurança
