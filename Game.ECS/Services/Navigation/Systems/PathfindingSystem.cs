    ...
    WorldMap mapData)
{
        ...
        PathfindingContext ctx)
    {
        ...
        // Validação rápida
        if (!IsValidCoord(request.StartX, request.StartY, request.StartZ) ||
            !IsValidCoord(request.GoalX, request.GoalY, request.GoalZ) ||
            !IsWalkable(request.StartX, request.StartY, request.StartZ) || 
            !IsWalkable(request.GoalX, request.GoalY, request.GoalZ))
        {
