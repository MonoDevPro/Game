...
namespace Game.Domain.Navigation.Core;
...
public sealed class NavigationGrid
{
    ...
    /// <summary>
    /// Força ocupação de uma célula, liberando qualquer ocupante anterior.
    /// Use com cuidado - pode causar inconsistências se o ocupante anterior não for notificado.
    /// </summary>
    /// <returns>EntityId do ocupante anterior, ou -1 se estava livre.</returns>
    public int ForceOccupy(GridPosition pos, int entityId)
    {
        if (!IsValid(pos)) return -1;

        int index = CoordToIndex(pos);
        return Interlocked.Exchange(ref _occupancy[index], entityId);
    }
