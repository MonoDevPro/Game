...
namespace GameWeb.Application.Common.Mapping;
...
public static class MappingExtensions
{
    ...
    public static Task<List<TDestination>> ProjectToListAsync<TDestination>(this IQueryable queryable, IMapper mapper,
        CancellationToken cancellationToken = default) where TDestination : class
        => queryable.ProjectTo<TDestination>(mapper.ConfigurationProvider).AsNoTracking()
            .ToListAsync(cancellationToken);

    public static Task<TDestination?> ProjectToSingleOrDefaultAsync<TDestination>(this IQueryable queryable,
        IMapper mapper, CancellationToken cancellationToken = default) where TDestination : class
        => queryable.ProjectTo<TDestination>(mapper.ConfigurationProvider).AsNoTracking()
